function varargout = spm_prf_analyse_JS(mode,varargin)
% Runs a PRF analysis
%
...
% JS, 17/08/2023:  'X0',X0, ...           % matrix of nuisance regressors
...
% ---------------------------------------------------------------------
% edited by Joram Soch <soch@cbs.mpg.de>, 17/08/2023, 12:06 (see "JS")
% ---------------------------------------------------------------------
...
TR   = SPM.xY(1).RT;
Y.dt = TR;
% check for nuisance variables --- JS, 17/08/23
if isfield(options,'X0')
    % average nuisance variables across sessions
    if options.avg_sess
        Y.X0 = mean(options.X0,3);
    % concatenate nuisance variables along sessions
    else
        Y.X0 = reshape(permute(options.X0,[1 3 2]),[n_sess*size(options.X0,1), size(options.X0,2)]);
    end;
else
    % include implicit baseline regressor
